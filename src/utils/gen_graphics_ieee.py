...
label_map = {
    ...
    'Fisher': 'Fisher Score', 'ReliefF': 'ReliefF', 'Baseline': 'Baseline'
}
...
def load_data(dataset_name):
    ...
    return {
        ...
        'Baseline': pd.read_csv(f'{path}/baseline_summary.csv'),
    }
...
def plot_metrics_together(x, dataset_name, dataset_label):
    ...
    for i in range(4):
        ...
        for idx, (label, data) in enumerate(methods.items()):
            data = data.iloc[:9, :] # Último elemento é com 100% das features
        ...
        ax.set_xlabel('Porcentagem (\%) de atributos selecionados', fontsize=14)
        ax.set_ylabel(y_labels[i], fontsize=14)
        # ax.set_title(y_labels[i], fontsize=11, weight='bold')
        ax.set_title(f'Avaliação de {y_labels[i]}', fontsize=18, weight='bold')
        ax.set_xticks(x)
